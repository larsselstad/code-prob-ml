# source: https://github.com/probml/pyprobml/blob/master/notebooks/book1/13/sparse_mlp.ipynb
...
from jax.example_libraries.stax import Dense, Softplus
...
def pgd(alpha, lambd):
    ...
    def update(i, g, w):
        ...
        updated_params = soft_thresholding(
            w_flat - step_size * g_flat, step_size * lambd
        )
...
def line_search(w, g, batch, beta):
    ...
    for i in range(20):
        is_converged = loss(z, batch) > loss(w, batch) + g_flat @ (
            z_flat - w_flat
        ) + np.sum((z_flat - w_flat) ** 2) / (2 * lr_i)
...
plt.savefig("figures/sparse_dnn/sparse_mlp_fit.png", dpi=300)
